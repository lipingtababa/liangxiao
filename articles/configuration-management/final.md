...
但现在有了Docker和IaC，保证环境一致性很容易了。如果有配置项消失了，那肯定是个bug，直接fail fast就好了。默认值和fallback机制，是前云时代的遗留思维。
...
当配置散落在.env、数据库、代码默认值、YAML文件多个地方，AI很难判断优先级。它只能说"likely"，因为它不确定runtime时哪个配置会生效。这种隐式的覆盖逻辑，AI处理不好，人类也容易出错。
